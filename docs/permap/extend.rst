...
To complete the performance map, Fillomino relies on correction curves,
...
to the attribute :attr:`~PermapFiller.mode`.

.. warning::
   When using :class:`PermapFiller` attributes or methods,
   don't forget to use the ``pmf`` prefix !
...
The entries are stored in the :attr:`~PermapFiller.entries` attribute, in the
form of a :class:`dict` with input quantities as keys and entries as values.
...
>>> permap.pmf.entries['freq'] = [0.1, 0.5, 0.9]
...
:ref:`entries <set entries>`, in the
:attr:`~PermapFiller.corrections` attribute.
...
you could use :py:`permap.pmf.corrections['freq']['power']` (see examples in
the :class:`PermapFiller` docstring). There is also a dedicated getter method,
:meth:`~PermapFiller.get_correction`, that would return the same correction
with :py:`permap.pmf.get_correction('freq', 'power')`.
...
:mod:`~fillomino.defaults` module.
For getting a specific correction, there is the function
:func:`~fillomino.defaults.default_correction`.
As with the :meth:`PermapFiller.get_correction` method, it returns a callable
...
:attr:`~PermapFiller.mode` arranged in a :class:`dict` structure as shown above,
using the function :func:`~fillomino.defaults.build_default_corrections`.
...
:attr:`~PermapFiller.corrections` attribute, but you may want to use the
:meth:`~PermapFiller.set_correction` method instead. Indeed, it performs some
...
There is also the :meth:`~PermapFiller.set_corrections` method, which allows
you to specify all corrections associated with an input quantity. In addition,
...
``cop`` and ``power``, you can do ::
   ...
   new_permap = permap.pmf.set_corrections('freq', new_corrections)

and :py:`new_permap.pmf.corrections['freq']` should have an entry ``'capacity'``.
...
using the attribute :attr:`~PermapFiller.initial_norm_values`, where values are
...
>>> permap.pmf.initial_norm_values['freq'] = 120 / 75
...
:meth:`~PermapFiller.fill` the performance map. To do so, simply

>>> permap_full = permap.pmf.fill()

Note that you can combine the :meth:`~PermapFiller.fill` operation with the
:meth:`~PermapFiller.normalize` operation (see :ref:`normalizing data <norm>`),
by providing rated values to the :meth:`~PermapFiller.fill` method.
